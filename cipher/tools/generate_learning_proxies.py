#!/usr/bin/env python3
"""Build harmless CIL* selector proxies for safe Cipher power learning."""
...
def rows(path: Path) -> list[tuple[str, str]]:
    ...
    for line in lines[3:]:
        ...
        if len(fields) < 3:
            raise ValueError(f"malformed CIPICK row: {line}")
...
def neutralize(source: bytes) -> bytes:
    ...
    # Preserve UI metadata, but never preserve hostile-cast semantics from the
    # real power. Learning a power must not break sanctuary/invisibility.
...
def build(override: Path, output: Path, pick: Path) -> int:
    ...
    for power, proxy in rows(pick):
        ...
        if not source.exists():
            source = override / f"{power}.spl"
        if not source.exists():
            raise FileNotFoundError(f"missing Cipher power {power}.SPL")
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path(".cipher-learn-build"))
    parser.add_argument("--pick", type=Path, default=Path("cipher/tables/cipick.2da"))
    parser.add_argument("--cleanup", action="store_true")
    args = parser.parse_args()
    if args.cleanup:
        shutil.rmtree(args.output, ignore_errors=True)
        return
    print(f"Generated {build(args.override, args.output, args.pick)} Cipher learning proxies")

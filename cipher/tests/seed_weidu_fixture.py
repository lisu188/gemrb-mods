...
def write_item(path, attack_types):
    header_offset = 0x72
    effect_offset = header_offset + 0x38 * len(attack_types)
    data = bytearray(effect_offset)
    data[:8] = b"ITM V1  "
    struct.pack_into("<H", data, 0x1C, 19)
    struct.pack_into("<I", data, 0x64, header_offset)
    struct.pack_into("<H", data, 0x68, len(attack_types))
...
write_item(override / "CIFHIT.ITM", (1, 2))
write_item(override / "CIFMAGIC.ITM", (3,))

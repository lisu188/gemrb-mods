#!/usr/bin/env python3
"""Core static, fake-runtime, and GUI-hook checks for the GemRB Psion mod."""
...
def validate_builders() -> None:
    ...
    for include in (
        "level1-powers.tpa", "level2-powers.tpa", "level3-powers.tpa",
        "level4-powers.tpa", "energy-ray-augment.tpa",
    ...
    assert set(re.findall(r"ps_resref = ~(PS1[A-Z0-9]+)~", level1)) == LEVEL1
    assert set(re.findall(r"ps_resref = ~(PS2[A-Z0-9]+)~", level2)) == LEVEL2
    assert "WRITE_LONG 0x34 1" in level1
    assert "WRITE_LONG 0x34 2" in level2
    assert "WRITE_LONG 0x18 ps_flags" in level1
    assert "WRITE_LONG 0x18 ps_flags" in level2
    ...
    for resref, fragments in level1_required.items():
    ...
    for resref, fragments in level2_required.items():
    ...
    assert "parameter1 = 4" in affinity and "duration = 60" in affinity
...
def validate_augmentation() -> None:
    ...
    for parent, (filename, count) in selectors.items():
        ...
        assert all(row[2] == "3" for row in data)
...
def validate_runtime() -> None:
    ...
    try:
        ...
        assert set(module.available_variants(1, "PS2AAFF")) == {
            "PSAASTR", "PSAADEX", "PSAACON"
        }
...
def main() -> None:
    ...
    validate_runtime()
    validate_patcher()
    print("Psion core progression, level-1/2, runtime, and GUI-hook validation passed.")

...
- Rejects duplicate `SORCERER_MONK` identity rows in `CLSKILLS.2DA` or the active class table instead of letting table lookup order choose an ambiguous class ID.
...
- Requires valid `XPCAP.2DA` entries for both Sorcerer and Monk before deriving the multiclass cap, instead of accepting a partial component table.
